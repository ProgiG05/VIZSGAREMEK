...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const TypesGroupingContainer = document.getElementById(
    "plant-type-grouping-cont",
  );

  // Build type filter buttons
  const AllPlantTypes = ["Saved plants"];
  response.forEach((onetype) => {
    if (!AllPlantTypes.includes(onetype.type)) {
      AllPlantTypes.push(onetype.type);
    }
  });
  ...
  AllPlantTypes.forEach((onetype) => {
    ...
    TypeButton.addEventListener("click", async () => {
      if (TypeButton.id === "Saved plants") {
        if (!user) {
          showAlert(
            "Please log in to view your saved plants.",
            "Not logged in!",
          );
          return;
        }
        ...
        try {
          ...
          savedPlants.forEach((plant) =>
            PlantsContainer.appendChild(createPlantCards(plant)),
          );
        } catch (error) {
          console.error("Failed to load saved plants:", error.message);
          showAlert(
            "Could not load saved plants. Please try again later.",
            "Error!",
          );
        }
      } else {
  ...
  response.forEach((oneorigin) => {
    if (!AllOrigins.includes(oneorigin.origin)) {
      AllOrigins.push(oneorigin.origin);
    }
  });
  ...
  document
    .getElementById("clear-water-btn")
    .addEventListener("click", () => clearFilters("water"));
  document
    .getElementById("clear-sunlight-btn")
    .addEventListener("click", () => clearFilters("sunlight"));
  document
    .getElementById("clear-soil-btn")
    .addEventListener("click", () => clearFilters("soil"));
});
...
function createPlantCards(plant) {
  ...
  addTableRow(
    "Soil type:",
    `${capitalizeFirstLetter(plant.soil)} nutrient density`,
  );
  ...
  switch (plant.type.toLowerCase()) {
    case "succulents":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.succulents})`,
      );
      break;
    case "herbs":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.herbs})`,
      );
      break;
    case "flowers":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.flowers})`,
      );
      break;
    case "ornamental":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.ornamental})`,
      );
      break;
    case "grass":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.grass})`,
      );
      break;
    case "vegetables":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.vegetables})`,
      );
      break;
    case "fruits":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.fruits})`,
      );
      break;
    case "ferns":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.ferns})`,
      );
      break;
    case "mosses":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.mosses})`,
      );
      break;
    case "trees":
      addTableRow(
        "Water quantity:",
        `${capitalizeFirstLetter(plant.water)}:  (${plantWateringMetrics.trees})`,
      );
      break;
  }
...
document
  .getElementById("searchPlant_Btn")
  .addEventListener("click", async (e) => {
    e.preventDefault();

    // Target values mapping
    const commonNameSearch = document
      .getElementById("commonplant-search-inp")
      .value.toLowerCase()
      .trim();
    const botanicalNameSearch = document
      .getElementById("botanicalplant-search-inp")
      .value.toLowerCase()
      .trim();
    const water =
      document.querySelector(".water-radioBtn:checked")?.value.toLowerCase() ||
      null;
    const sunlight =
      document
        .querySelector(".sunlight-radioBtn:checked")
        ?.value.toLowerCase() || null;
    const soil =
      document.querySelector(".soil-radioBtn:checked")?.value.toLowerCase() ||
      null;
    const plantingMonth = document
      .getElementById("plantingSelection")
      .value.toLowerCase();
    const harvestingMonth = document
      .getElementById("harvestingSelection")
      .value.toLowerCase();
    const origin = document
      .getElementById("originSelection")
      .value.toLowerCase();
    ...
    try {
      ...
      const filteredPlants = plants.filter((p) => {
        const criteriaCommon =
          !commonNameSearch ||
          p.common_name.toLowerCase().includes(commonNameSearch);
        const criteriaBotanical =
          !botanicalNameSearch ||
          p.botanical_name.toLowerCase().includes(botanicalNameSearch);
        const criteriaWater = !water || p.water.toLowerCase() === water;
        const criteriaSunlight =
          !sunlight || p.sunlight.toLowerCase() === sunlight;
        const criteriaSoil = !soil || p.soil.toLowerCase() === soil;
        const criteriaPlanting =
          plantingMonth === "none" ||
          !plantingMonth ||
          p.planting.toLowerCase().includes(plantingMonth);
        const criteriaHarvesting =
          harvestingMonth === "none" ||
          !harvestingMonth ||
          p.harvesting.toLowerCase().includes(harvestingMonth);
        const criteriaOrigin =
          origin === "none" || !origin || p.origin.toLowerCase() === origin;
      ...
      const searchedPlantsContainer = document.getElementById(
