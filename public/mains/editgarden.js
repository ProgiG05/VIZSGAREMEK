...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const RowColumnManageSidePanel = document.getElementById(
    "RowColumnManageSidePanel",
  );
...
function EditGarden(garden, plants, parentContainer, controls) {
  ...
  manageRowsColumnsBtn.onclick = () =>
    ManageRowsColumns(garden, plants, parentContainer, controls);
  ...
  resetBtn.onclick = async () => {
    if (
      await showConfirm(
        "Are you sure you want to reset this garden? Any unsaved changes will be lost.",
      )
    ) {
  ...
  function tempSaveGardenData() {
    const gardenTableData = Array.from(table.querySelectorAll("tr"))
      .map((row) => {
        return Array.from(row.querySelectorAll("td"))
          .map((cell) => {
            ...
            const plant = plants.find(
              (p) => p.common_name === cell.textContent,
            );
...
function ManageRowsColumns(garden, plants, parentContainer, controls) {
  ...
  removeLastRowBtn.onclick = async () => {
    if (
      await showConfirm(
        "Are you sure you want to remove the last row from this garden? Any unsaved changes will be lost.",
      )
    ) {
      let rows = garden.garden_content.split(";");
  ...
  removeLastColumnBtn.onclick = async () => {
    if (
      await showConfirm(
        "Are you sure you want to remove the last column from this garden? Any unsaved changes will be lost.",
      )
    ) {
      let rows = garden.garden_content.split(";");
      let canRemove = true;
